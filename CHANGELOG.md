...
## [Unreleased](https://github.com/kalel1500/laravel-db-sync/compare/v0.9.1-beta.2...master)

## [v0.9.1-beta.2](https://github.com/kalel1500/laravel-db-sync/compare/v0.9.1-beta.1...v0.9.1-beta.2) - 2026-07-14

### Fixed

* Oracle identity synchronization no longer applies `GENERATED BY DEFAULT AS IDENTITY` for `BY DEFAULT` columns.
  * The `BY DEFAULT` generation mode is now normalized to `GENERATED BY DEFAULT ON NULL AS IDENTITY`.
  * This preserves compatibility with Laravel Eloquent + `yajra/laravel-oci8` when inserts omit the primary key or pass `NULL` for the identity column.
